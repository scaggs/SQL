...
WHERE (Drug_Group = 'ANTINEOPLASTICS AND ADJUNCTIVE THERAPIES' OR Drug_Class = 'Immunomodulators')
AND Route_of_Administration = 'OR' AND MultiSource IN ('N','M') -- single-source oral oncology drugs
...
WHERE p.month >= '2017-01-01 00:00:00.000'
...
GROUP BY contact.cntct_key, submitting_user_id, tru.username, tru.organization_name, tru.office_contact, tru.display_name, geo.cbsa_nm_tx, tru.email, tru.phone, tru.user_type, tru.city, tru.state, tru.software_vendor, sg.segment_category, tru.requests, tru.real_requests, tru.account_id, tru.created_on, tru.last_touch, tru.account_name
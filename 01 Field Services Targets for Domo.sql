...
LEFT JOIN Sandbox.dbo.t_ehr_epic_healthorg_domainlist_adhoc epic ON LOWER (RIGHT (tru.email, LEN(tru.email) - CHARINDEX('@', tru.email)+1))=epic.email_domain /* Matches up Epic systems with their sales exec */
LEFT JOIN CMM_DataMart.dbo.cmm_dim_geo geo ON LEFT(tru.zip, 5)=geo.zip_cd /* Matches the user's zip code and NOT the doc's NPI zip code to ensure proper CBSA attribution */
